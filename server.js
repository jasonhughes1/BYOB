...
const checkAuth = (request, response, next) => {
  if (environment !== 'test') {
    ...
    jwt.verify(requestToken, secretKey, (error) => {
      if (error) {
...
const httpServer = app.listen(app.get('port'), () => {
  console.log(`byob running on ${app.get('port')}`);  //eslint-disable-line no-console
});
  ...
  database('photos').where('nasa_id', nasa_id).select()
    .then(photo => {
      if (photo[0]) {
        ...
        return response.status(404).json({
          error: `No photo with nasa_id of ${nasa_id} was found.`});
      }
    })
    .catch(error => {
      return response.status(500).json({ error });
